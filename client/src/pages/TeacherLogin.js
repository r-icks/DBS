...
const TeacherLogin = () => {
  ...
  const handleLogin = () => {
    setupUser({ currentUser: { teacherId: teacherID, password }, endpoint: 'teacherLogin' });
  };

  const handleRegister = () => {
    setupUser({
      currentUser: { teacherId: teacherID, teacherEmail: email, teacherName: name, password },
  ...
  return (
    ...
    <Alert />
      <div className="bg-white p-8 rounded-lg shadow-md text-center">
        <h2 className="text-2xl font-semibold mb-4">{isMember ? 'Teacher Login' : 'Teacher Registration'}</h2>
        ...
        <p className="text-blue-500 cursor-pointer hover:underline mt-4" onClick={() => setIsMember(!isMember)}>
